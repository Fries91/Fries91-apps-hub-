...
(function () {
    ...
    root.innerHTML = `
      <div id="${HUB_OVERLAY_ID}">
        ...
        <div class="thub-body">
          ...
          <div class="thub-note">
            Open each tool from here. Extra corner launchers stay hidden so Torn stays clean.
          </div>
        </div>
      </div>
  ...
  function renderHubVisibility() {
    ...
    if (shield) {
      shield.style.display = (!state.hubOpen && !isAnyAppOverlayOpen()) ? 'inline-flex' : 'none';
    }
  }
...
try {
    ...
    root.innerHTML = `
      <div id="${HUB_OVERLAY_ID}">
        ...
        <div class="thub-body">
          ...
          <div class="thub-note">
            War Hub is embedded in this merged build. Insurance and Lottery can be plugged in next.
          </div>
        </div>
      </div>
      <div id="${HUB_APP_LAYER_ID}"></div>
  ...
  function renderHubVisibility() {
    ...
    if (shield) {
      shield.style.display = state.hubOpen ? 'none' : 'inline-flex';
    }
  }
  ...
  function registerDemoApps() {
    registerApp({
      ...
      name: 'War Hub',
      icon: '⚔️',
      description: 'Real War and Chain overlay with backend, tabs, and live war tools.',
      open: ({ createWindow }) => {
        openWarHubModule(createWindow);
      },
    });
      ...
      name: 'Sinner Insurance',
      icon: '💊',
      description: 'Real insurance overlay with plans, claims, activations, War Stack, and backend sync.',
      open: ({ createWindow }) => {
        if (window.__FRIES_INSURANCE_BRIDGE__ && typeof window.__FRIES_INSURANCE_BRIDGE__.open === 'function') {
          try {
            const launcher = document.getElementById('si-pda-launcher');
      ...
      name: 'Giveaway / Lottery',
      icon: '🎟️',
      description: 'Real giveaway overlay with entry flow, entrants, winners, admin controls, and countdowns.',
      open: ({ createWindow }) => {
        if (window.__FRIES_GIVEAWAY_BRIDGE__ && typeof window.__FRIES_GIVEAWAY_BRIDGE__.open === 'function') {
          try {
            const launcher = document.getElementById('giveaway-shield');
  ...
  try { boot(); } catch (e) { console.error('Torn Hub boot failed', e); }
  try { startMountWatch(); } catch (e) { console.error('Torn Hub watch failed', e); }
})();
